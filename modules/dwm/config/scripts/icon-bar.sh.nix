  ...
  # Append a space after the icon within the quotes, then you'll see what icon is it.
    ...
    wlan() {
      ...
      # cat /sys/class/net/enp4s0/operstate 2>/dev/null
    	...
    	up)   printf "^c${base01}^ ^b${base0E}^ $wlan_icon_succes ^d^%s" "^b${base01}^ ^c${base07}^ Connected" ;;
    	down) printf "^c${base01}^ ^b${base0E}^ $wlan_icon_fail ^d^%s" "^b${base01}^ ^c${base07}^ Disconnected" ;;
    	esac
    }
    ...
    timeinfo() {
    	printf "^c${base01}^ ^b${base06}^ $time_icon "
    	printf "^c${base01}^^b${base05}^  $(date '+%R')  "
    }
